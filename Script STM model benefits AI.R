...
# benefits
data <- dat[ ,c("X", "gender_bin", "benefits_ai_trade")] |>
  filter(!is.na(gender_bin))
data$text <- stri_trans_tolower(data$benefits_ai_trade)
...
#7 was selected as it has the highest semantic coherence and the fewer residuals while keeping the other indicators in the average. I set gender_bin as the content to study how gender influences the choice of each words INSIDE the topics.
...
for (i in 1:7) {
  plot(stm_model, type = "perspectives", topics = i)
}